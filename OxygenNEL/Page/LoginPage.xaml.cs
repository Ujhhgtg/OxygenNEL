...
public sealed partial class LoginPage : Microsoft.UI.Xaml.Controls.Page
{
    ...
    private async void LoginButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                NotificationHost.ShowGlobal("请输入用户名或邮箱", ToastLevel.Warning);
                return;
            }

            if (string.IsNullOrWhiteSpace(pass))
            {
                NotificationHost.ShowGlobal("请输入密码", ToastLevel.Warning);
                return;
            }
    ...
    private async void SendCodeButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsQQNumber(qq))
            {
                NotificationHost.ShowGlobal("QQ号格式不正确", ToastLevel.Warning);
                return;
            }
    ...
    private async void NextStepButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                NotificationHost.ShowGlobal("验证码必须是6位数字", ToastLevel.Warning);
                return;
            }
    ...
    private async void RegisterButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (!_registerCodeVerified)
            {
                NotificationHost.ShowGlobal("请先完成邮箱验证码验证", ToastLevel.Warning);
                return;
            }
            ...
            {
                NotificationHost.ShowGlobal("用户名长度必须在 3-64 字符", ToastLevel.Warning);
                return;
            }

            if (password.Length < 8)
            {
                NotificationHost.ShowGlobal("密码至少8位", ToastLevel.Warning);
                return;
            }

            if (!string.Equals(password, password2, StringComparison.Ordinal))
            {
                NotificationHost.ShowGlobal("两次输入的密码不一致", ToastLevel.Warning);
                return;
            }
